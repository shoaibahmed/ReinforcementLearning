...
while ~converged
    ...
    visitCountsCurrentEpisode = zeros(size(v));
    ...
    for i = 1 : length(episode)
        ...
        if ~visitCountsCurrentEpisode(state(1), state(2))
            ...
            visitCountsCurrentEpisode(state(1), state(2)) = visitCountsCurrentEpisode(state(1), state(2)) + 1;
        end
    end
...
if converged
    fprintf('First visit Monte-Carlo policy evaluation algorithm converged to the solution in %d iterations\n', iterations);
else
