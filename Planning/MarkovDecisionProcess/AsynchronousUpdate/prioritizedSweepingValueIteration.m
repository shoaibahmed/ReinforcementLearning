...
while ~converged
    ...
    plotValues(bellmanError, strcat(['Bellman Error (', num2str(iterations), ' iterations)']));
    ...
    % Store for computing bellman error
    previousV = v;
    
    % Perform synchronous value iteration to get a reasonable estimate of the importance of each state
    if iterations == 0
        ...
        bellmanError = abs(previousV - v);
        bellmanError(~maze) = 0;
        
    else
        % Update the state value function according to bellman error
        % Get the term with most bellman error to be used as i and j
        updated = false;
        while ~updated
            [value, index] = max(bellmanError(:));
            [i, j] = ind2sub(size(bellmanError), index);
            ...
            if maze(i, j)
                ...
                % Update the bellman error
                bellmanError(i, j) = abs(previousV(i,j) - v(i,j));
